...
window.CONTRACT = {
  address: "0x430Ee9ea7B289Bbd4492DC39E8DD5A503Bf9e75E",
...
async function get_Sha3() {
  $("#note").html(`<h5 class="text-warning">Encrypting Your Document...</h5>`);
  ...
  if (file) {
    ...
    reader.onload = async function (evt) {
      ...
      $("#note").html(
        `<h5 class="text-center text-info">Do you want to Upload document to IPFS and store hash on Blockchain?</h5>`
      );
    };
...
async function print_verification_info(result, is_verified) {
    // Hide loader
    $("#loader").fadeOut();

    // Reset student document image
    $("#student-document").attr("src", is_verified ? "./files/valid.svg" : "./files/notvalid.svg");

    // Remove old messages
    $("#verification-status").html("");
    $("#confirmation-text").remove();
    $("#explanation-text").remove();
    $("#not-verified-text").remove();

    // Hide "Do you want to proceed?" prompt
    $(".proceed-text").hide();

    // Disable the Verify button
    $("#upload_file_button").prop("disabled", true);

    // Hide other blockchain fields
    $("#transaction-hash").hide();
    $("#time-stamps").hide();
    $("#blockNumber").hide();
    $("#blockHash").hide();
    $("#to-network").hide();
    ...
    // Placeholder for new read-only blockchain transaction display
    $("#transaction-details").remove(); // remove old one if exists
  $("#file-hash").after(`<div id="transaction-details" class="transaction-details-transparent"></div>`);



    if (!is_verified) {
        // Not verified case: show only one clear message
        $("#file-hash").html(`
            <h6 class="text-Success text-start">
                Certificate Not Verified – File Not Found on the Blockchain.
                <i class="fa fa-times-circle text-danger"></i>
            </h6>
            <h6 class="text-Success text-start">
                Try again by selecting the Verify tab and uploading the original document.
            </h6>
        `);
        // Hide contract-address as well
    ...
    // Verified case
    $("#file-hash").html(`
        <p class="text-success text-start mb-1">
            File Verified! Successfully saved on the Blockchain via IPFS.
        </p>
        <span class="text-info"><i class="fa-solid fa-hashtag"></i></span> ${truncateAddress(window.hashedfile)}
    `);

    $("#contract-address").html(result.contractAddress || "N/A").hide();

    $("#contract-address").after(`
        <p class="text-muted mb-2 text-start">
           
        </p>
          <h6 class="text-Success text-start" <center>
                Click the Upload Tab above to try another Document.
            </h6>
    `);

    // Show verification status heading
 $("#verification-status").html(`
    <div class="verification-status-bg">
...
`);
    ...
    // ✅ Show Sepolia transaction details (read-only)
    const txHash = "0x0f53b525778399f774183d85015757b8a412a00b8c8254d994331e8978eab33a"; // Example hash
    await showTransactionDetails(txHash);
}

// Helper function for fetching transaction data
async function showTransactionDetails(txHash) {
    const apiKey = "YOUR_ETHERSCAN_API_KEY"; // Replace with your Etherscan API key
    const apiUrl = `https://api-sepolia.etherscan.io/api?module=transaction&action=gettxreceiptstatus&txhash=${txHash}&apikey=${apiKey}`;
    const txUrl = `https://api-sepolia.etherscan.io/api?module=proxy&action=eth_getTransactionByHash&txhash=${txHash}&apikey=${apiKey}`;
    ...
    try {
        ...
        if (!txRes.result) {
            $("#transaction-details").html(`<p class="text-danger">Unable to fetch transaction details.</p>`);
            return;
        }

        const tx = txRes.result;
        const receiptStatus = receiptRes.result?.status === "1" ? "Success" : "Failed";
        const blockNumber = parseInt(tx.blockNumber, 16);
        const gasPrice = parseInt(tx.gasPrice, 16) / 1e9; // in Gwei
        const valueEth = parseInt(tx.value, 16) / 1e18; // in ETH

        const txHtml = `
    <div class="transaction-info mt-4 p-3 border rounded bg-light">
        <strong class="mb-3 text-success"><i class="fa-solid fa-circle-check"></i> Blockchain Transaction Details</strong>
...
`;
        ...
        $("#transaction-details").html(txHtml);

    } catch (error) {
        console.error(error);
        $("#transaction-details").html(`<p class="text-danger">Error loading transaction data.</p>`);
    }
}
...
function printUploadInfo(result) {
  ...
  $("#note").html(`<h5 class="text-info" style="color: #00ff00 !important;>
   Transaction Saved to the BlockChain <i class="mx-2 text-info fa fa-check-circle"  aria-hidden="true"></i>
   </h5>`);
  listen();
}
...
async function sendHash() {
  ...
  $("#note").html(
    `<h5 class="text-center text-info" style="color: #00ff00 !important;">Please confirm the transaction</h5>`
  );
  $("#upload_file_button").attr("disabled", true);
  get_ChainID();
  // Initilize Ipfs
  ...
  if (window.hashedfile.length > 4) {
    await window.contract.methods
      ...
      .on("transactionHash", function (_hash) {
        $("#note").html(
         `<h5 class="text-center text-info" style="color: #00ff00 !important;">Processing transaction... Mining in progress. . .</h5>`
        );
      })
...
async function deleteHash() {
  ...
  $("#upload_file_button").slideUp();
  $("#note").html(
    `<h5 class="text-info" style="color: #00ff00 !important;">Please confirm the transaction</h5>`
  );
  $("#upload_file_button").attr("disabled", true);
  get_ChainID();
  ...
  if (window.hashedfile) {
    await window.contract.methods
      ...
      .on("receipt", function (receipt) {
        $("#note").html(
          `<h5 class="text-center text-info" style="color: #00ff00 !important;">Document Deleted to Blockchain</h5>`
        );
...
async function get_ChainID() {
  ...
  switch (a) {
    ...
    case 42:
      ...
      break;
    default:
      window.chainID = "Sepolia ChainID";
      break;
  }
  ...
  if (network) {
    document.getElementById(
      "network"
    ).innerHTML = `<i class="text-success fa-solid fa-circle-nodes mx-2"></i>${window.chainID}`;
  }
}
...
async function editExporter() {
  ...
  if (info && address) {
    ...
    try {
      await window.contract.methods
        ...
        .on("receipt", function (receipt) {
          ...
          console.log(receipt);
          $("#note").html(
            `<h5 class="text-info" style="color: #00ff00 !important;">Exporter Updated Successfully</h5>`
          );
        })
    ...
    $("#note").html(
      `<h5 class="text-center text-warning" style="color: #00ff00 !important;">You need to provide address & information to update </h5>`
    );
  }
}
...
async function deleteExporter() {
  ...
  if (address) {
    ...
    $("#note").html(
      `<h5 class="text-center text-warning" style="color: #00ff00 !important;">You need to provide address to delete</h5>`
    );
  }
}
...
document.getElementById("blockchain-status").classList.remove("d-none");
